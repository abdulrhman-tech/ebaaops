...
import {
  ...
  Package, Users, Hash, FileText, TrendingUp, Eye, XCircle,
} from "lucide-react";
...
export default function CategoryReportDetailPage() {
  const [, params] = useRoute("/reports/category/:name");
  const categoryName = decodeURIComponent(params?.name ?? "");
  usePageTitle(`تقرير الصنف: ${categoryName}`);

  const { token, user } = useAuth();
  const { isRTL } = useLang();
  const statusLabels = getStatusLabels({ requestDetail: {} } as any);
  ...
  const { data, isLoading } = useQuery<CategoryDetailData>({
    queryKey: ["/api/reports/category-detail", categoryName],
    queryFn: async () => {
      const res = await fetch(`/api/reports/category-detail/${encodeURIComponent(categoryName)}`, {
  ...
  const uniqueReps = useMemo(() => {
    ...
    return Array.from(new Set(data.requests.map(r => r.salesRepName)));
  }, [data]);
  ...
  return (
    <div className="flex flex-col h-full" dir={isRTL ? "rtl" : "ltr"}>
      <Topbar title={isRTL ? `تقرير الصنف: ${categoryName}` : `Category Report: ${categoryName}`} />
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">{isRTL ? "تفاصيل حجوزات هذا الصنف" : "Reservation details for this category"}</p>
        </div>
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {[
                ...
                { icon: <Package className="w-4 h-4 text-amber-500" />, label: isRTL ? "إجمالي الكمية المحجوزة" : "Total Reserved Qty", value: data.kpis.totalReservedQty.toLocaleString() },
            ...
            <Card>
              <CardHeader className="pb-2">
                <h3 className="text-sm font-semibold">{isRTL ? "المناديب الحاجزين لهذا الصنف" : "Sales Reps Who Reserved This Category"}</h3>
              </CardHeader>
